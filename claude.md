...
- Work big enough to break things mid-flight → the **`EnterWorktree` tool** (this line
  ...
  work along. `.claude/settings.json` sets `worktree.baseRef: "head"`, so check
...
Test one file: `bun test {path}`. Do not run the full suite when a scoped run covers your change — full runs cost minutes, and peers' in-flight edits make unrelated failures look like yours.
